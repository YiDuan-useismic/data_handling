...
def merge_stream_by_id(
    ...
    *,
    ...
    """
    Merge multiple traces with the same trace.id into single traces.

    Assumptions:
    - All traces with the same ID have identical sampling rates.
    - Gaps represent genuinely missing data.

    Parameters
    ----------
    stream
        Input stream possibly containing multiple segments per trace.id.
    fill_value
        If None, gaps are preserved and traces remain split.
        If a float (e.g. 0.0), gaps are padded with that value.

    Returns
    -------
    Stream
        Stream with at most one trace per trace.id.
    """
    merged = stream.copy()

    # method=1: conservative merge, respects gaps
    merged.merge(method=1, fill_value=fill_value)
...
def export_overlapping_windows_across_files(
    ...
    raw_stream = merge_stream_by_id(raw_stream)
    aligned_stream = align_stream_to_union(raw_stream)
